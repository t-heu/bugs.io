...
export default function GameArena({ character, onGameOver }: any) {
  ...
  const createBot = (x: number, y: number, type = null) => {
  ...
  useEffect(() => {
    ...
    // Generate bots - place them in a visible pattern
    const initialBots: any = []
    const botTypes = ["ant", "spider", "beetle"]

    // Posicionar bots em um padrão de grade ao redor do jogador
    const gridSize = Math.ceil(Math.sqrt(BOT_COUNT))
    const spacing = 100
    const startX = ARENA_SIZE / 2 - (gridSize * spacing) / 2
    const startY = ARENA_SIZE / 2 - (gridSize * spacing) / 2
    ...
    for (let i = 0; i < BOT_COUNT; i++) {
      ...
      // Alternar tipos de insetos para variedade
      const botType: any = botTypes[i % botTypes.length]

      initialBots.push(createBot(x, y, botType))
    }

    console.log(`Criados ${initialBots.length} bots`)
    setBots(initialBots)
    setDebugInfo((prev) => ({ ...prev, botCount: initialBots.length }))
  ...
  }, []);  // Dependências vazias, para rodar apenas uma vez quando o componente é montado
